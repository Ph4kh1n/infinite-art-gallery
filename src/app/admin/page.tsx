...
export default function AdminPage() {
  ...
  const cursorReset = (
    <style>{`
      #admin-root, #admin-root * { cursor: auto !important; }
      #admin-root button, #admin-root [role="button"], #admin-root input[type="button"] { cursor: pointer !important; }
    `}</style>
  );

  const content = !authed ? (
    <div id="admin-root" className="fixed inset-0 flex items-center justify-center">
      <div className="w-full max-w-sm mx-6">
        ...
        />
        {authError && (
          <p className="mt-3 font-mono text-xs text-red-500">{authError}</p>
        )}
        <button
          onClick={handleAuth}
    ...
    <div id="admin-root" className="min-h-screen bg-background-light dark:bg-background-dark">
      <div className="max-w-xl mx-auto px-6 py-12">
        <h1 className="font-display text-display-sm text-foreground-light dark:text-foreground-dark">Manage Images</h1>
        <p className="font-mono text-xs text-muted mt-2">JPG, PNG, GIF &middot; Max {MAX_MB}MB each</p>
        ...
        {results && (
          <div className="mt-6 space-y-1">
            {results.map((r, i) => (
              <p key={i} className={`font-mono text-xs ${r.status === 'uploaded' ? 'text-green-600 dark:text-green-400' : 'text-red-500'}`}>
